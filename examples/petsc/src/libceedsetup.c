...
PetscErrorCode SetupLibceedBDDC(DM dm_Pi, CeedData data_fine,
  ...
  CeedOperator op_Pi_r, op_r_Pi, op_Pi_Pi, op_r_r, op_r_r_inv, op_inject_Pi,
               op_inject_r, op_restrict_Pi, op_restrict_r;
  CeedVector x_Pi_ceed, y_Pi_ceed, mult_ceed, x_r_ceed, y_r_ceed, z_r_ceed,
  ...
  CeedInt strides[3] = {num_comp_u, 1, num_comp_u*elem_size};
  ...
  for (CeedInt e=0; e<num_elem; e++) {
    for (CeedInt n=0; n<elem_size; n++) {
      ...
      for (CeedInt c=0; c<num_comp_u; c++) {
        CeedInt index = strides[0]*n + strides[1]*c + strides[2]*e;
