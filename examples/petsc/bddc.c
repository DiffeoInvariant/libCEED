...
int main(int argc, char **argv) {
  ...
  PC pc;
  Mat mat_O, mat_S_Pi;
  Vec X, X_loc, X_Pi, X_Pi_loc, rhs, rhs_loc;
  ...
  ierr = MatCreateShell(comm, l_Pi_size, l_Pi_size, g_Pi_size,
                        g_Pi_size, user_bddc, &mat_S_Pi); CHKERRQ(ierr);
  ierr = MatShellSetOperation(mat_O, MATOP_MULT,
                              (void(*)(void))MatMult_BDDCSchur); CHKERRQ(ierr);
  ierr = MatShellSetVecType(mat_S_Pi, vec_type); CHKERRQ(ierr);
  ...
  // Set up PCShell user data (also used for Schur MatShell)
  user_bddc->comm = comm;
  user_bddc->dm = dm;
